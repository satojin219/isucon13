...
func getUserStatisticsHandler(c echo.Context) error {
	...
	if err := tx.SelectContext(ctx, &rankingData, query); err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to fetch ranking data: "+err.Error())
	}

	// ランキング計算
	var ranking UserRanking
	for _, data := range rankingData {
		score := data.Reactions + data.Tips
